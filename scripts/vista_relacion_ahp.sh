...
mysql -u "${user}" --password="${pass}" --execute="USE ${DB};SELECT ESTUDIO.TIPO AS ESTUDIO, t3.TIPO AS DATO, CONCAT('= ',t1.MAGNITUD,' veces ',t4.TIPO) AS 'RELATIVO', t1.REGISTRO FROM RELEVANCIA_DATOS t1 LEFT JOIN RELEVANCIA_DATOS t2 ON (t1.TIPO_DATO_ESTUDIO = t2.TIPO_DATO_ESTUDIO AND t1.REGISTRO < t2.REGISTRO) LEFT JOIN TIPO_DATO_ESTUDIO t3 ON(t3.tdID=t1.TIPO_DATO_ESTUDIO AND t3.tdID!=t1.RELATIVO) LEFT JOIN TIPO_DATO_ESTUDIO t4 ON(t4.tdID!=t1.TIPO_DATO_ESTUDIO AND t4.tdID=t1.RELATIVO) JOIN (ESTUDIO) ON(ESTUDIO.testID=t1.ESTUDIO) WHERE t2.REGISTRO IS NULL ORDER BY t3.TIPO,t1.MAGNITUD DESC;"  | column -t -s $'\t' >${temp}"tmp2.ed"
