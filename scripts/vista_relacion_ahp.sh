...
function menu_seleccion {
	...
	case $exit_status in
		...
		cat "./"${temp}"/export.ed" >""${inf}"/"${DATA_COMPOSC}"" && dialog --msgbox "INFORMACION DISPONIBLE EN ARCHIVO: "${DATA_COMPOSC}"" 0 0
		;;
...
mysql -u "${user}" --password="${pass}" --execute="USE ${DB};SELECT ESTUDIO.TIPO AS ESTUDIO,t1.TIPO_DATO_ESTUDIO AS ID, t3.TIPO AS DATO,t1.MAGNITUD,t4.TIPO AS 'RELATIVO',t1.RELATIVO AS ID, t1.REGISTRO FROM RELEVANCIA_DATOS t1 LEFT JOIN RELEVANCIA_DATOS t2 ON (t1.TIPO_DATO_ESTUDIO = t2.TIPO_DATO_ESTUDIO AND t1.RELATIVO = t2.RELATIVO AND t1.REGISTRO < t2.REGISTRO) LEFT JOIN TIPO_DATO_ESTUDIO t3 ON(t3.tdID=t1.TIPO_DATO_ESTUDIO AND t3.tdID!=t1.RELATIVO) LEFT JOIN TIPO_DATO_ESTUDIO t4 ON(t4.tdID!=t1.TIPO_DATO_ESTUDIO AND t4.tdID=t1.RELATIVO) JOIN (ESTUDIO,JERARQUIA) ON(ESTUDIO.testID=t1.ESTUDIO AND JERARQUIA.TIPO_DATO_ESTUDIO=t1.TIPO_DATO_ESTUDIO) WHERE t2.REGISTRO IS NULL ORDER BY JERARCA;"  | tr '\t' ';' >${temp}"export.ed"
