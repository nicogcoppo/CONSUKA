...
function muestra_particulares {

    mysql -u "${user}" --password="${pass}" --execute="USE ${DB};SELECT SUBSTRING(ESTUDIO.TIPO,1,4) AS ESTUDIO, MODELO.MODELO AS AERONAVE, TIPO_DATO_ESTUDIO.TIPO AS DATO, IF(t1.MAGNITUD<.01,CONCAT(1 / t1.MAGNITUD,' ',UNIDADES.UNIDAD,' INVERTIDO'),CONCAT(t1.MAGNITUD,' ',UNIDADES.UNIDAD)) AS MAGNITUD, t1.REGISTRO FROM DATO_ESTUDIO t1 JOIN(UNIDADES,ESTUDIO,TIPO_DATO_ESTUDIO,MODELO) ON(t1.MODELO=MODELO.moID AND t1.UNIDADES=UNIDADES.unidID AND t1.ESTUDIO=ESTUDIO.testID AND t1.TIPO_DATO_ESTUDIO=TIPO_DATO_ESTUDIO.tdID) LEFT JOIN DATO_ESTUDIO t2 ON(t1.TIPO_DATO_ESTUDIO=t2.TIPO_DATO_ESTUDIO AND t1.MODELO=t2.MODELO AND t1.REGISTRO<t2.REGISTRO) WHERE t2.REGISTRO IS NULL AND TIPO_DATO_ESTUDIO.TIPO LIKE "%"${VAR_ESP}"%" ORDER BY IF(t1.MAGNITUD<0.01,1 / t1.MAGNITUD,t1.MAGNITUD) DESC;" >${temp}"tmp2.ed"
...
mysql -u "${user}" --password="${pass}" --execute="USE ${DB};SELECT ESTUDIO.TIPO AS ESTUDIO, MODELO.MODELO AS AERONAVE, TIPO_DATO_ESTUDIO.TIPO AS DATO, IF(t1.MAGNITUD<.01,CONCAT(1 / t1.MAGNITUD,' ',UNIDADES.UNIDAD,' INVERTIDO'),CONCAT(t1.MAGNITUD,' ',UNIDADES.UNIDAD)) AS MAGNITUD, t1.REGISTRO FROM DATO_ESTUDIO t1 JOIN(UNIDADES,ESTUDIO,TIPO_DATO_ESTUDIO,MODELO) ON(t1.MODELO=MODELO.moID AND t1.UNIDADES=UNIDADES.unidID AND t1.ESTUDIO=ESTUDIO.testID AND t1.TIPO_DATO_ESTUDIO=TIPO_DATO_ESTUDIO.tdID) LEFT JOIN DATO_ESTUDIO t2 ON(t1.TIPO_DATO_ESTUDIO=t2.TIPO_DATO_ESTUDIO AND t1.MODELO=t2.MODELO AND t1.REGISTRO<t2.REGISTRO) WHERE t2.REGISTRO IS NULL ORDER BY MODELO.MODELO,TIPO_DATO_ESTUDIO.TIPO;"  | tr '\t' ';' >${temp}"export.ed"
